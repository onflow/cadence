...
func TestCheckInvalidEntitlementMappingAuth(t *testing.T) {
	...
	t.Run("invalid variable declaration type", func(t *testing.T) {
		t.Parallel()

		_, err := ParseAndCheck(t, `
            entitlement mapping M {}

            let x: auth(mapping M) &Int = &3 as &Int
        `)
	...
	t.Run("invalid function parameter type", func(t *testing.T) {
		t.Parallel()
	...
	t.Run("invalid reference expression type", func(t *testing.T) {
		t.Parallel()
	...
	t.Run("invalid casting expression type", func(t *testing.T) {
		t.Parallel()
	...
	t.Run("invalid invocation type argument", func(t *testing.T) {
		t.Parallel()
	...
	t.Run("fun ref field", func(t *testing.T) {
		...
		_, err := ParseAndCheck(t, `
			...
			fun test(foo: fun(auth(mapping M) &Int): auth(mapping M) &Int) {}
        `)
...
func TestCheckEntitlementInheritance(t *testing.T) {
	...
	t.Run("mismatched mapped interfaces", func(t *testing.T) {
		...
		assert.IsType(t, &sema.InterfaceMemberConflictError{}, errs[0])
	})
...
func TestCheckEntitlementMapAccess(t *testing.T) {
	...
	t.Run("basic, invalid", func(t *testing.T) {
		t.Parallel()
	...
	t.Run("basic with optional access, invalid", func(t *testing.T) {
		t.Parallel()
	...
	t.Run("unrepresentable disjoint", func(t *testing.T) {
		...
		assert.IsType(t, &sema.UnrepresentableEntitlementMapOutputError{}, errs[0])
	})
	...
	t.Run("owned set field", func(t *testing.T) {
		t.Parallel()
	...
	t.Run("reference set field", func(t *testing.T) {
		t.Parallel()
		...
		assert.IsType(t, &sema.InvalidAssignmentAccessError{}, errs[0])
		// TODO: type mismatch error for assignment?
	})
}
...
func TestCheckIdentityMapping(t *testing.T) {
	...
	t.Run("owned value", func(t *testing.T) {
		...
		_, err := ParseAndCheck(t, `
            struct S {
                access(mapping Identity) let foo: [Int]
	...
	t.Run("unauthorized reference", func(t *testing.T) {
		t.Parallel()
		...
		_, err := ParseAndCheck(t, `
            ...
            fun main() {
                ...
                let resultRef1: &AnyStruct = sRef.foo
                let resultRef2: auth(Insert) &AnyStruct = sRef.foo
            }
        `)
	...
	t.Run("authorized reference", func(t *testing.T) {
		...
		_, err := ParseAndCheck(t, `
            ...
            fun main() {
                let sRef = &S() as auth(Insert) &S

                let resultRef1: &AnyStruct = sRef.foo
                let resultRef2: auth(Insert) &AnyStruct = sRef.foo
                let resultRef3: auth(Mutate) &AnyStruct = sRef.foo
            }
        `)
	...
	t.Run("entitlement set ref", func(t *testing.T) {
		...
		_, err := ParseAndCheck(t, `
            ...
            fun main() {
                ...
                let resultRef1: auth(Insert | Remove) &[Int] = ref1.foo

                let ref2 = &s as auth(Insert, Remove) &S
                let resultRef2: auth(Insert, Remove) &[Int] = ref2.foo
            }
        `)
...
func TestCheckNestedReferenceAuthorizationIntersection(t *testing.T) {
	...
	t.Run("array, unauth outer, auth inner, index access", func(t *testing.T) {
		t.Parallel()
		...
		_, err := ParseAndCheck(t, `
          ...
          fun test() {
              ...
              // Outer is unauthorized, inner is auth(E).
              // Intersection is empty, so result is &Int.
	...
	t.Run("array, auth outer, auth inner, matching entitlements", func(t *testing.T) {
		t.Parallel()
		...
		_, err := ParseAndCheck(t, `
          ...
          fun test() {
              var s = 1
              let array: [auth(E) &Int] = [&s as auth(E) &Int]
              let arrayRef = &array as auth(E) &[auth(E) &Int]

              // Outer auth(E) intersect inner auth(E) = auth(E).
              let ref: auth(E) &Int = arrayRef[0]
          }
        `)
	...
	t.Run("array, partial intersection", func(t *testing.T) {
		...
		_, err := ParseAndCheck(t, `
          ...
          fun test() {
              ...
              // Outer auth(E, F) intersect inner auth(F, G) = auth(F).
	...
	t.Run("array, partial intersection, escalation prevented", func(t *testing.T) {
		...
		_, err := ParseAndCheck(t, `
          ...
          fun test() {
              ...
              // Outer auth(E, F) intersect inner auth(F, G) = auth(F).
              // Cannot assign auth(F) to auth(F, G).
	...
	t.Run("array, disjoint entitlements", func(t *testing.T) {
		...
		_, err := ParseAndCheck(t, `
          ...
          fun test() {
              var s = 1
              let array: [auth(F) &Int] = [&s as auth(F) &Int]
              let arrayRef = &array as auth(E) &[auth(F) &Int]

              // Outer auth(E) intersect inner auth(F) = empty.
              let ref: &Int = arrayRef[0]
          }
        `)
	...
	t.Run("dictionary, auth outer, auth inner, matching entitlements", func(t *testing.T) {
		t.Parallel()
		...
		_, err := ParseAndCheck(t, `
          ...
          fun test() {
              var s = 1
              let dict: {String: auth(E) &Int} = {"a": &s as auth(E) &Int}
              let dictRef = &dict as auth(E) &{String: auth(E) &Int}
	...
	t.Run("dictionary, unauth outer, auth inner", func(t *testing.T) {
		...
		_, err := ParseAndCheck(t, `
          ...
          fun test() {
              var s = 1
              let dict: {String: auth(E) &Int} = {"a": &s as auth(E) &Int}
              let dictRef = &dict as &{String: auth(E) &Int}

              // Outer unauthorized intersect inner auth(E) = unauthorized.
	...
	t.Run("composite, no mapping, unauth outer, auth inner", func(t *testing.T) {
		t.Parallel()
	...
	t.Run("composite, no mapping, auth outer, auth inner, matching", func(t *testing.T) {
		t.Parallel()
	...
	t.Run("composite, no mapping, auth outer, auth inner, disjoint", func(t *testing.T) {
		t.Parallel()
		...
		_, err := ParseAndCheck(t, `
          ...
          fun test() {
              ...
              // Outer auth(E) intersect inner auth(F) = empty (E ≠ F).
              let ref: &Int = sRef.x
          }
        `)
	...
	t.Run("composite, no mapping, auth outer, auth inner, partial intersection", func(t *testing.T) {
		t.Parallel()
		...
		_, err := ParseAndCheck(t, `
          ...
          struct S {
              ...
              init() {
                  var i = 1
                  self.x = &i as auth(F, G) &Int
              }
          }

          fun test() {
              var s = S()
              let sRef = &s as auth(E, F) &S
