...
export class CadenceDocgen {

  private static functionNamePrefix = "CADENCE_DOCGEN"
  private static loaded = false

  private static functionName(name: string): string {
    return `__${CadenceDocgen.functionNamePrefix}_${name}__`
  }

  public static async create(binaryLocation: string | BufferSource): Promise<CadenceDocgen> {
    await this.ensureLoaded(binaryLocation)
    return new CadenceDocgen()
  }
  ...
  public generate(code: string): any {
    const result = global[CadenceDocgen.functionName('generate')](code)
