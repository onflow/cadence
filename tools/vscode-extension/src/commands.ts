...
import {
  FlowAccountTreeItem,
  FlowAccountDetailTreeItem,
} from "./explorer/accounts-data-provider";
  ...
  setTimeout(async () => {
    try {
      const accounts = await ext.api.createDefaultAccounts(
        ext.config.accounts.numAccounts - 1
      );
      accounts.forEach((address) => ext.config.accounts.addAccount(address));
    } catch (err) {
...
const stopEmulator = (ext: Extension) => async () => {
  ...
  // state is in sync.
  ext.config.accounts.resetAccounts();
  renderExtension(ext);
  await ext.api.client.stop();
  ext.api = new LanguageServerAPI(ext.ctx, ext.config);
};
...
const createAccount = (ext: Extension) => async () => {
  try {
    ...
    ext.config.accounts.addAccount(addr);
  } catch (err) {
...
const switchActiveAccount = (ext: Extension) => async () => {
  ...
  const accountOptions = Object.values(ext.config.accounts.list)
    // Mark the active account with a `*` in the dialog
    .map((account) => {
      const suffix: String =
        account.index === ext.config.accounts.activeAccount
          ? ` ${activeSuffix}`
          : "";
  ...
  window.showQuickPick(accountOptions).then((selected) => {
    ...
    const activeAccount = ext.config.accounts.getAccount(activeIndex);
    ...
    ext.config.accounts.setActiveAccount(activeIndex);
