...
More importantly, risks associated with ceding control to an external contract are in fact only an Ethereum concern;
the risk no longer applies because controls are handled with Capabilities.
