...
Fields, functions, Resources and/or Capabilities in a contract use [access modifiers](access-control)
...
Remote access to stored objects can be managed via [Capabilities](capability-based-access-control). This
...
[Fungible Token](../flow#flow-token)(FT) and [Non-Fungible Token](../flow#flow-nft#overview)(NFT)
...
Detailed information about available standards and other core contracts can be found in [Introduction to Flow](../flow).

### NFT standard and metadata

Solidity must manage NFT metadata off-chain and NFTs typically link to IPFS JSON from on-chain.

The Cadence NFT standard provides in-built support for metadata with specific types called [views](../flow#flow-nft#overview#nft-metadata).
Views can be added to NFTs when minted and will always be available as part of the NFT. While metadata is stored
on-chain, graphics and video content is still stored off-chain. Cadence provides [utility views](../flow#flow-nft#overview#list-of-common-views)
...
NFT metadata on Flow opens the door to exciting new possibilities that other chains can't achieve. Check out a
recent [case study](https://flow.com/post/flovatar-nft-flow-blockchain-case-study) where one of our community
partners leveraged SVG on-chain NFT metadata to enable 2D + 3D versions without any off-chain data!
...
[Capabilities](capability-based-access-control) are defined by linking storage paths (namespaces for contract
...
[PublicAccount](accounts#publicaccount)/[AuthAccount](accounts#authaccount) account scopes.
...
to allow any account to `borrow()` a reference to the Vault to make a `deposit()`. Since only the functions defined
under the `FungibleToken.Receiver` interface are exposed, the borrower of the vault reference cannot call `withdraw()`
since it is scoped in the `provider` interface. Public capabilities can be obtained from both
...
to another account which places in the [account inbox](accounts#account-inbox). The recipient can later
...
Detailed information can be found in [Capabilities](capability-based-access-control).
...
Cadence. Functions are private by default and the language provides a range of [access scopes](access-control).
...
same way were it imported.

Detailed information about deploying, updating, removing or borrowing contracts can be found in [Contracts](contracts)
...
Flow uses [events](events) extensively to provide real-time signals to off-chain systems about particular
...
[Optional bindings](control-flow) provide in-built conditional handling of nil values. Regular data types in
...
Cadence offers the [Dictionary](control-flow) type, an unordered collection of key-value associations
which is iterable.
