...
language primitives such as Resources and Capabilities codify the complexity of access and security in first-class
types.
...
Optionals in Cadence are values which can represent the absence of a value. Optionals have two cases: either there is
a value, or there is nothing. Cadence offers optional bindings which help avoid faulty assumptions in code. They also
provide developers with a way to manage non-existent values rather than null.
