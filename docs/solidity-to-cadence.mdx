...
are always stored in accounts, any data that exists in Resource instances is seamlessly managed on-chain
...
`FlowToken.Vault` Resource. This vault is exposed as a public capability, scoped to the `FungibleToken.Receiver` resource interface,
