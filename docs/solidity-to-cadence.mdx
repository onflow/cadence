...
There are numerous contract standards established to benefit the ecosystem, for example
[Fungible Token](https://github.com/onflow/flow-ft/blob/master/contracts/FungibleToken.cdc) and
[Non-Fungible Token](https://github.com/onflow/flow-nft/blob/master/contracts/NonFungibleToken.cdc) are conceptually
equivalent to Ethereum's ERC-20 and ERC-721 standards. Cadence's object-oriented design means standards apply through
contract sub-types such as Resources, Resource interfaces, or other types declared in the contract standard.
Standards can define and limit behaviour and/or set conditions which implementations of the standard cannot violate.
...
Capabilities are either public (any user can get access) or private (access to/from the authorized user is necessary).
Capability privacy scopes correspond to [PublicAccount](language#accounts#publicaccount)/[AuthAccount](language#accounts#authaccount)
account privacy scopes mentioned earlier.

Public capabilities are created using public paths, i.e. they have the domain `public`. After creation, they can
be obtained from both authorized accounts (`AuthAccount`) and public accounts (`PublicAccount`).

Detailed information can be found in [Capabilities](language#capability-based-access-control).

Private capabilities are created using private paths, i.e. they have the domain `private`. After creation, they
can be obtained from authorised accounts (`AuthAccount`) but not public accounts (`PublicAccount`). To share a private
Capability with another account, the owning account must `publish` it to another account which places in the
[account inbox](language#accounts#account-inbox). The recipient can later claim the Capability from the account inbox
using then `claim` function.

Capabilities can be `unpublished` and private capabilities can be revoked by the creating account.

To aid automation events are emitted for `publish`, `claim` and `unpublish` actions used with a Capability.

## Delegation controls

Consider the following use cases:

* Alice wants to place an offer on a NFT in the ecosystem
