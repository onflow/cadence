...
within a Resource. The use of a Resource in this context is key - the instance can't be copied, and it will always be
stored in an account. This approach to admin roles also makes possible fine-grained control such as limiting
access to individual functions, on a per-account basis if required. Implementors can decide whether admin roles are
simple or complex, for example, larger organizations or groups may require a role-based-access scheme. If needed the
admin Resource can also be implemented to create additional admin Resource instances, which can be granted to
selected user accounts via a Capability. Conveniently, because the admin role is only accessible via its Capability,
it's easy to revoke access with [Capability Revocation](design-patterns#capability-revocation).

The admin role originates from the [init singletone pattern](design-patterns#init-singleton) and uses
[Capability Bootstrapping](design-patterns#capability-bootstrapping) pattern for granting the Capability.
