...
While capabilities grant account access to a protected resource, it's still necessary to limit how much value to make
available through them. For example, if your use-case requires providing another account access to a
`FLOWToken.Vault` to `withdraw()` funds via it's `provider` interface, it's important to limit the amount that might
be withdrawn. Tokens implementing FT/NFT standards are the primary type of value being exchanged by accounts on Flow.
The standard provides the primitives needed to implement capability limiting best-practices.
...
also eliminate confusion around the order of arguments when working with the same type.
