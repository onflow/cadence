...
to create a secure and declarative model for digital ownership by ensuring that resources (which are used to represent scarce assets)
...
Cadence’s syntax is inspired by [Swift](https://developer.apple.com/swift/) and [Rust](https://www.rust-lang.org/)
...
because many blockchains do not allow smart contracts to be updated or fixed once they are deployed,
so any bugs in the contract are unable to be fixed on many blockchains.
