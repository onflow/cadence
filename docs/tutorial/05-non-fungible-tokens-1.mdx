...
This allows NFTs to benefit from the resource ownership rules that are enforced by the [type system](https://docs.onflow.org/cadence/language/values-and-types/) -
...
[Hello, World!](/cadence/tutorial/02-hello-world/),[Resources](/cadence/tutorial/03-resources), and [Capabilities](/cadence/tutorial/04-capabilities)
...
These protections ensure that owners know that their NFT is safe and can represent an asset that has real value.
For more information about resources, see the [resources tutorial](/cadence/tutorial/03-resources)
...
transaction {
    prepare(signer1: AuthAccount, signer2: AuthAccount) {
        ...
        let nft <- signer1.load<@BasicNFT>(from: /storage/BasicNFTPath)

        // Save the NFT to signer2's account
        signer2.save<@BasicNFT>(<-nft, to: /storage/BasicNFTPath)
...
Continue on to the [next tutorial](/cadence/tutorial/05-non-fungible-tokens-2)
