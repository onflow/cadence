...
Most of the [account storage API](https://docs.onflow.org/cadence/language/accounts/#account-storage-api) is not available on the `PublicAccount` object, and only signed transactions can access the `AuthAccount` object.
