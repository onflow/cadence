...
The concepts involved in implementing a fungible token in Cadence can be hard to grasp at first.
...
pub resource Vault: Provider, Receiver {
    ...
    // because they can represent decimals and do not require the concept of a negative number
...
When interacting with resources, you use the `@` symbol to specify the type and a special “move operator” `<-` in assignments.
...
We are also using unsigned numbers in this example, so the vault`s balance cannot go below 0.
...
In the real world, you'll likely be transferring tokens to other accounts.
...
This feature ensures that, while the withdraw function is public,
...
This is especially important for array and dictionary types, which can have their fields maliciously assigned to
if they are made public. See the cadence best practices document for more info: 
...
Additionally, `ExampleToken` defines the `VaultMinter` resource which is used as the sole method to mint new tokens.
...
meaning that path names could potentially conflict. This is why it is important
...
This syntax might be confusing to you:
```cadence
