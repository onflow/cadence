...
- The `public` domain is your account's public API. It is exactly like the `private` domain in that
  the owner can link capabilities here that point to stored assets. 
  Anyone in the network can access the `public` domain's functionality.
