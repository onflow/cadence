...
1. The first area is the [contract area](/cadence/language/accounts).
   ...
   This area cannot be directly accessed in a transaction unless the transaction is just reading a copy of the code deployed to an account.
...
The account filesystem functions similar to a traditional filesystem;
Data is stored in paths, links can be created between paths that define different permissions and functionality,
and only the owner of the filesystem has "root" access to modify any of the data or links.
...
### The three main account filesystem domains (Where can I store my stuff?)
...
that signed the transaction and can read and write to those domains,
...
They must be explicitly moved from one location to another when accessed.
They also cannot go out of scope at the end of a function execution.
They must be explicitly stored somewhere or destroyed.
These characteristics make it impossible to lose accidentally from a coding mistake.
