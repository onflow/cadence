...
- The `public` domain is kind of like your account's public API. It is exactly like the `private` domain in that
...
1. The `prepare` phase is required, but is not used here, so it will be covered later.
...
Let's walk through this contract, starting with the resource, one of the most important things
...
are not an ideal way to represent this ownership because they can be copied.
