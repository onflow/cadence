...
Some may choose to authenticate or perform operations for their users by using their account addresses.
...
pub fun transferNFT(id: UInt64, owner: AuthAccount) {
    ...
    // BAD
    contractVault.deposit(<-stolenTokens)
}
...
```
...
An Auth Reference allows downcasting any restricted type to its unrestricted type.
...
could be happen since the value could be downcasted to a type
...
that borrows an auth reference to an NFT (eg. [NBA Top Shot](https://github.com/dapperlabs/nba-smart-contracts/blob/master/contracts/TopShot.cdc#L889-L906)).
...
Some NFTs may have admin functions that shouldn't be exposed by this method,
so please be cautious & mindful when imitating NFT projects that use this pattern.

## Events From Resources Might Not Be Unique

### Problem

Public functions in a contract can be called by any account. 
...
This anti-pattern is fixed with [FLIP #703](https://github.com/onflow/flow/blob/master/flips/20211129-cadence-mutability-restrictions.md)
