...
Don't trust the users’ [account storage](/cadence/language/accounts#account-storage). Users have full control over their data and may reorganize it as they see fit. Users may store values in any path, so paths may store values of “unexpected” types. These values may be instances of types in contracts that the user deployed.
...
Access to an AuthAccount gives full access to the storage, keys, and contracts. Therefore, [avoid using AuthAccount](cadence/anti-patterns#problem) as a function parameter unless absolutely necessary.
...
If it is necessary to handle the case where borrowing a capability might fail, the check function can be used to verify that the target exists and has a valid type.
