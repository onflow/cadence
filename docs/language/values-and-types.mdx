...
- `cadence•fun contains(key: K): Bool`
  ...
  // Check if the dictionary contains "twentyFive".
  let containsTwentyFive = numbers.contains("twentyFive")
  // `containsTwentyFive` is `false`

  // Check if the array contains "fortyTwo".
  let containsFortyTwo = numbers.contains("fortyTwo")
  // `containsFortyTwo` is `true`

  // Invalid: Check if the dictionary contains the integer 42.
  // This results in a type error, as they key type of the dictionary is string.
  //
  let contains42 = numbers.contains(42)
  ```
