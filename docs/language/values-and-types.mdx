...
- `cadence•fun containsKey(key: K): Bool`
  ...
  ```
- `cadence•fun forEachKey(function: ((K): Bool)): Void`
  ...
  fun myContainsKey(targetKey: String, dictionary: {String: Int}) {
    ...
    dictionary.forEachKey(fun(key: String): Bool {
      ...
      // The returned boolean value, signals whether to continue iterating. This allows for control flow during the iteration process:
      // true = `continue`
      // false = `break`
      return !found
    })
