...
- `cadence•fun replace(from: String, to:String): String`
  Returns a string where all hits of `from` is replaced with `to`  

	```cadence
  let example = "Flowers"

  example.replace(from: "wers", to: "vatar")  // is `Flovatar`
	```
