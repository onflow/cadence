...
## Identity
Identity can only be obtained as a reference from an AuthAccount and has a single `address` field. 

The main use case for Identity is to send it from a transaction into a contract method to know the address of who called it. 

```cadence

// example fun 
pub fun someMethod(identity: &Identity) {
   log("I was called by ".concat(identity.address))
}

transaction() {
    prepare(signer: AuthAccount) {
				someMethod(signer.identity)
    }
}
