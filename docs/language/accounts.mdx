...
<Callout type="info">
⚠️  Note: Keys can be added also using the `addPublicKey` function. However, this method is currently
deprecated and is available only for the backward compatibility. The `addPublicKey` method accepts
the public key encoded together with their signature algorithm, hashing algorithm and weight.
```cadence
...
```
<Callout type="info">
⚠️  Note: Keys can also be removed using the `removePublicKey` function. However, this method is
deprecated and is available only for the backward compatibility.
```
</Callout>
