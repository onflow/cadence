...
func TestRuntimeImportedValueMemoryMetering(t *testing.T) {
	...
	t.Run("Optional", func(t *testing.T) {
		...
		assert.Equal(t, ifCompile[uint64](9, 4), meter[common.MemoryKindOptionalStaticType])
	})
	...
	t.Run("Word256", func(t *testing.T) {
		...
		meter := make(map[common.MemoryKind]uint64)
		executeScript(t, script, meter, cadence.NewWord256(2))
		assert.Equal(t, uint64(32), meter[common.MemoryKindBigInt])
	})
	...
	t.Run("Struct", func(t *testing.T) {
		...
		assert.Equal(t, ifCompile(ifTracing[uint64](142+216, 71+216), ifTracing[uint64](142, 71)), meter[common.MemoryKindRawString])
	})
	...
	t.Run("InclusiveRange", func(t *testing.T) {
		...
		assert.Equal(t, uint64(1), meter[common.MemoryKindCompositeValueBase])
		assert.Equal(t, ifCompile[uint64](2, 1), meter[common.MemoryKindInclusiveRangeStaticType])
...
func TestRuntimeImportedValueMemoryMeteringForSimpleTypes(t *testing.T) {
	...
	tests := []importTest{
		...
		},
		{
			...
			Weight:     ifCompile[uint64]((1+3)+(1+3)+216, (1+3)+(1+3)),
			TypeInstance: cadence.Path{
				Domain:     common.PathDomainStorage,
				Identifier: "id3",
			},
...
func TestRuntimeScriptDecodedLocationMetering(t *testing.T) {
	...
	tests := []importTest{
		...
		{
			MemoryKind: common.MemoryKindRawString,
			Weight:     ifCompile[uint64](0x9c, 12),
			Name:       "string",
