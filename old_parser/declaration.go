...
func parseAccess(p *parser) (ast.PrimitiveAccess, error) {

	switch string(p.currentTokenSource()) {
	case keywordPriv:
		// Skip the `priv` keyword
		p.next()
		return ast.AccessSelf, nil

	case keywordPub:
		// Skip the `pub` keyword
		p.nextSemanticToken()
		if !p.current.Is(lexer.TokenParenOpen) {
			return ast.AccessAll, nil
		}
	...
	case keywordAccess:
		...
		var access ast.PrimitiveAccess
...
func parseSpecialFunctionDeclaration(
	...
	case keywordDestroy:
		declarationKind = common.DeclarationKindDestructorLegacy
